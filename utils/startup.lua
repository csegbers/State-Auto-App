...
--=======================================================
--== User Defined Info Get and Put
--==
--== Get and Put 1 column at a time. Put in Table
--== Will only flag fields that changed
--== only changed fields will be updated at end
--==
--== [as in table key name... not the field name... that wil be retreived
--======================================================= 
...
function myApp.fncCCDBFlushUpdates()
    if myApp.mydb.dbref and myApp.mydb.dbref:isopen() then
       ...
       for key,value in pairs(myApp.mydb.lCC) do
            print ("Key",key,"value",value.value,"dirty",value.dirty)
            if value.dirty == true then
                value.dirty = false
                if upstr ~= nil then upstr = upstr .. ", " else upstr = "" end
                upstr = upstr .. key .. [[ = ']] .. value.value .. [[']]
                print ("Updating UD Table Key",key,"value",value.value,"dirty",value.dirty)
            end 
        end    -- for
        if upstr then
          local parms = myApp.mydb.dbconfig.dbschema.UD
...
--== run once at the end
--================================================
function myApp.fncExitApp (closedb)
...
function myApp.onSystemEvent( event )
   ...
   if event.type == "applicationStart" then
    elseif event.type == "applicationExit" then
        myApp.fncExitApp(true)
    elseif event.type == "applicationSuspend" then
        myApp.fncExitApp(false)
